...
get_strings() {
	...
	# lupdate -no-obsolete -ts uifiles.ts ../ui/*.ui
	# Get translatable strings from Python file(s)
	# pylupdate5 -verbose -ts pyfiles.ts ../../{InitGui,polyhedrons}.py
	pylupdate6 --no-obsolete -ts pyfiles.ts ../../{InitGui,polyhedrons}.py
	# Join strings from Qt Designer and Python files into a single temp file
	lconvert -sort-contexts -no-obsolete -i pyfiles.ts -o _${WB}.ts
}
